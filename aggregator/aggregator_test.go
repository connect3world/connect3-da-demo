...
import (
	...
	"github.com/connect3world/c3-data-avs/aggregator/mocks"
...
func TestSendNewTask(t *testing.T) {
	...
	mockAvsWriterer.EXPECT().SendNewTaskAddressToC3Data(
		context.Background(), ADDRESS_TO_FETCH_C3DATA, types.QUORUM_THRESHOLD_NUMERATOR, types.QUORUM_NUMBERS,
	).Return(mocks.MockSendNewTaskAddressToC3DataCall(BLOCK_NUMBER, TASK_INDEX, ADDRESS_TO_FETCH_C3DATA))
	...
	err = aggregator.sendNewTask(ADDRESS_TO_FETCH_C3DATA)
	assert.Nil(t, err)
}
