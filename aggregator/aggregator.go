...
type Aggregator struct {
	...
	tasks                 map[types.TaskIndex]cstaskmanager.IConnect3DATaskManagerTask
	tasksMu               sync.RWMutex
	taskResponses         map[types.TaskIndex]map[sdktypes.TaskResponseDigest]cstaskmanager.IConnect3DATaskManagerTaskResponse
...
func NewAggregator(c *config.Config) (*Aggregator, error) {
	...
	chainioConfig := sdkclients.BuildAllConfig{
		...
		RegistryCoordinatorAddr:    c.Connect3DARegistryCoordinatorAddr.String(),
	...
	return &Aggregator{
		...
		tasks:                 make(map[types.TaskIndex]cstaskmanager.IConnect3DATaskManagerTask),
		taskResponses:         make(map[types.TaskIndex]map[sdktypes.TaskResponseDigest]cstaskmanager.IConnect3DATaskManagerTaskResponse),
	}, nil
}
...
func (agg *Aggregator) Start(ctx context.Context) error {
	...
	taskNum := int64(0)
	addresses := []string{
		"0x523D3bd434b4E1F4F0268F6cC1f735C02f08C998",
		"0xd8da6bf26964af9d7eed9e03e53415d37aa96045",
		"0x18EE32610d8fe6E8f49041ce97Eecaf5E7A5b6cB",
	}
	addressIndex := 0
	getNextAddress := func() string {
		address := addresses[addressIndex]
		addressIndex = (addressIndex + 1) % len(addresses)
		return address
	}
	// ticker doesn't tick immediately, so we send the first task here
	// see https://github.com/golang/go/issues/17601
	_ = agg.sendNewTask(gethcommon.HexToAddress(getNextAddress()))
	taskNum++
	...
	for {
		...
		case <-ticker.C:
			err := agg.sendNewTask(gethcommon.HexToAddress(getNextAddress()))
			taskNum++
			if err != nil {
...
func (agg *Aggregator) sendNewTask(addressToFetchC3Data gethcommon.Address) error {
	agg.logger.Info("Aggregator sending new task", "addressToFetchC3Data", addressToFetchC3Data)
	// Send to the task manager contract
	newTask, taskIndex, err := agg.avsWriter.SendNewAddressToFetchC3Data(context.Background(), addressToFetchC3Data, types.QUORUM_THRESHOLD_NUMERATOR, types.QUORUM_NUMBERS)
	if err != nil {
		agg.logger.Error("Aggregator failed to send", "err", err)
		return err
	}
