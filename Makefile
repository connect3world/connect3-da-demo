...
# check in contracts/script/output/${CHAINID}/c3_da_avs_deployment_output.json
...
deploy-connect3-da-contracts-to-anvil-and-save-state: ## Deploy avs
	./tests/anvil/deploy-avs-save-anvil-state.sh

deploy-all-to-anvil-and-save-state: deploy-eigenlayer-contracts-to-anvil-and-save-state deploy-connect3-da-contracts-to-anvil-and-save-state ## deploy eigenlayer, shared avs contracts, and c3-da contracts
...
docker-build-and-publish-images: ## builds and publishes operator and aggregator docker images using Ko
	KO_DOCKER_REPO=ghcr.io/layr-labs/connect3-da ko build aggregator/cmd/main.go --preserve-import-paths
	KO_DOCKER_REPO=ghcr.io/layr-labs/connect3-da ko build operator/cmd/main.go --preserve-import-paths
...
start-aggregator: ## 
	go run aggregator/cmd/main.go --config config-files/aggregator.yaml \
		--c3-da-deployment ${DEPLOYMENT_FILES_DIR}/c3_da_avs_deployment_output.json \
